...
namespace Utils.Generics
{
    public interface IRepository<T> where T : class
    {
        ...
        Task<IResult> DeleteManyAsync(IEnumerable<T> entites);
    ...
    public abstract class Repository<T> : IRepository<T> where T : class
    {
        ...
        public async Task<IResult> DeleteManyAsync(IEnumerable<T> entites)
        {
            try
            {
                await Task.Run(() => _table.RemoveRange(entites));
                return Result.Success(204);
            }
            catch (Exception ex)
            {
